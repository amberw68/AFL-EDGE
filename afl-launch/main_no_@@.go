...
var (
	flagAFL   = flag.String("afl", "", "afl-fuzz option (afl)")
...
func main() {
	...
	if *flagNoMaster {
		name := baseName + "_" + "S" + "_" + "0"
		spawn(name, append(baseArgs, "-S", name))
	} else {
		name := baseName + "_" + "M" + "_" + "0"
		spawn(name, append(baseArgs, "-M", name))
	}

	// launch the rest
	for i := 1; i < *flagNum; i++ {
		name := baseName + "_" + "S" + "_" + strconv.Itoa(i)
		spawn(name, append(baseArgs, "-S", name))
	}
}
